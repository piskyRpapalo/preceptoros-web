...
/* --- Guía de Instalación Rápida (no pasa por IA) --- */
var btnGuia = document.getElementById('guia-rapida');
var zonaGuia = document.getElementById('guia-instalacion');
if (btnGuia && zonaGuia) {
  var comandos = {
    'Linux/macOS': 'git clone --depth 1 https://github.com/piskyRpapalo/PreceptorOS ~/aurelius\ncd ~/aurelius && bash bin/instalar-pc\npython3 preceptoros.py\nbin/preceptoros-servicio arranca',
    'Termux/Android': 'pkg install python git\ngit clone --depth 1 https://github.com/piskyRpapalo/PreceptorOS ~/aurelius\ncd ~/aurelius && python3 preceptoros.py',
    'Windows (WSL)': 'wsl\ngit clone --depth 1 https://github.com/piskyRpapalo/PreceptorOS ~/aurelius\ncd ~/aurelius && bash bin/instalar-pc\npython3 preceptoros.py'
  };
  
  btnGuia.addEventListener('click', function() {
    if (zonaGuia.style.display === 'none' || !zonaGuia.style.display) {
      var html = '<strong>Selecciona tu sistema:</strong><br><br>';
      for (var so in comandos) {
        html += '<button type="button" class="boton-so" data-so="' + so + '" style="margin:0.3rem;padding:0.4rem 0.8rem;background:#6D5AE0;color:#fff;border:1px solid #5A4531;cursor:pointer;font:inherit">' + so + '</button><br>';
      }
      zonaGuia.innerHTML = html;
      zonaGuia.style.display = 'block';
      
      // Añadir listeners a botones de SO
      setTimeout(function() {
        var botonesSO = zonaGuia.querySelectorAll('.boton-so');
        for (var i = 0; i < botonesSO.length; i++) {
          botonesSO[i].addEventListener('click', function() {
            var so = this.getAttribute('data-so');
            zonaGuia.innerHTML = '<strong>' + so + ':</strong><br><br><code style="background:#E4E8EC;padding:0.5rem;display:block;margin:0.5rem 0;border:1px solid #78624C">' + comandos[so].replace(/\n/g, '<br>') + '</code><br><button type="button" id="copiar-guia" class="boton" style="padding:0.4rem 0.8rem">Copiar comandos</button> <button type="button" id="cerrar-guia" style="padding:0.4rem 0.8rem;background:transparent;border:1px solid #78624C;cursor:pointer">Cerrar</button>';
            
            // Listener para copiar
            document.getElementById('copiar-guia').addEventListener('click', function() {
              navigator.clipboard.writeText(comandos[so]).then(function() {
                this.textContent = '¡Copiado!';
                setTimeout(function() { this.textContent = 'Copiar comandos'; }.bind(this), 2000);
              }.bind(this));
            });
            
            // Listener para cerrar
            document.getElementById('cerrar-guia').addEventListener('click', function() {
              zonaGuia.style.display = 'none';
            });
          });
        }
      }, 100);
